...
public class RespostaAlternativaServiceTests
{
    private readonly Mock<IRespostaAlternativaRepository> _mockRespostaAlternativaRepository;
    ...
    private readonly RespostaAlternativaService _service;

    public RespostaAlternativaServiceTests()
    {
        _mockRespostaAlternativaRepository = new Mock<IRespostaAlternativaRepository>();
        _mockInteracaoRepository = new Mock<IInteracaoRepository>();
        _mockQuestaoRepository = new Mock<IQuestaoRepository>();
        _mockRepository = new Mock<IRepository>();
        _mockMapper = new Mock<IMapper>();

        _service = new RespostaAlternativaService(
    ...
    public async Task Adicionar_DeveAdicionarRespostaAlternativa_QuandoDadosForemValidos()
    {
        ...
        var respostaModel = new RespostaAlternativaModel(requestDto, interacao, questao);
        ...
        _mockRespostaAlternativaRepository.Setup(x => x.Adicionar(It.IsAny<RespostaAlternativaModel>())).ReturnsAsync(respostaModel);
        _mockMapper.Setup(x => x.Map<RespostaAlternativaResponseDTO>(It.IsAny<RespostaAlternativaModel>())).Returns(new RespostaAlternativaResponseDTO(1, 1, 1, 2, null, questao, interacao));
